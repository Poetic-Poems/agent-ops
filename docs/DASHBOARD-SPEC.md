...
`scripts/autonomy-stage-report.sh` already provide. Omitted, it is the real
wall clock, exactly as before this option existed; it exists only so a test
can pin every window this script computes to a calendar date it controls,
rather than the clock the test happens to run under.
