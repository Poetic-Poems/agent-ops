...
| `max_chained_cycles` | `3` | Finish-then-continue (requirement 41): how many further selection cycles a cycle that did real work may chain immediately afterward, bounded so a busy fleet still yields the lock periodically. `1` disables chaining. |
