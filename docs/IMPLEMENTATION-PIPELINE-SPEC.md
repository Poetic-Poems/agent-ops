...
| `claim_ttl_hours` | `6` | Age beyond which `lib/claim.sh gc` sweeps a claim-registry entry — far beyond a whole cycle (120 min Implementor + 45 min Reviewer), so only a dead node's claim ever expires. The branch itself is deleted only if untouched and PR-less. |
...
| `lock_stale_after` | 4 h | Greater than the sum of the stage timeouts plus slack — 15 + 120 + 45 + 30 minutes once the Enabler can run inside the lock (requirement 35). |
