...
| `agent-cycle.sh` | 196 | Every hit read in context, in two line-range passes, then re-swept phrase by phrase (a `grep` per Band A pattern's own wording) because the line-range passes alone left residual sites behind | 12 |
