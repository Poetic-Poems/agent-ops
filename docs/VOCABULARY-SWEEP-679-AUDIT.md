...
|---|---:|---|---|
| `prompts/` (8 files) | 234 | Every hit in every file read in context. Missed `prompts/coordinator.md:108-114`'s own unconditional "a human's review" / "no human is waiting on us" framing of `review_feedback`, true only below `agent-approves` — above it the Approver App's own `REQUEST_CHANGES` populates that same array (`scripts/gather-review-feedback.sh` keys off `reviewDecision`, which counts both alike); caught and reworded in PR #748's review round instead | 41 |
| `docs/IMPLEMENTATION-PIPELINE-SPEC.md` | 569 | Every hit read in context, in four overlapping-free line-range passes, cross-checked against `agent-cycle.sh`'s real branching for escalation-gating claims, then re-swept phrase by phrase (a `grep` per Band A pattern's own wording) because the line-range passes alone left residual sites behind | ~27 |
| `lib/` (33 files) | 282 | Every hit in every file read in context — but, like `agent-cycle.sh`'s below, the pass reworded three body sites in `lib/handoff.sh` while leaving that file's own header (lines 3-9) asserting the same `merge_autonomy`-gated landing claim in the spelling nothing grepped for ("becomes the human's", "everything after it is a human's queue"); caught and reworded in PR #748's review round instead | 11 (1 added in that round) |
| `scripts/` (19 files) | 216 | Every hit in every file read in context | 1 (keeping the `merge_autonomy: human` rung per Edge 3) |
| `agent-cycle.sh` | 196 | Every hit read in context, in two line-range passes, then re-swept phrase by phrase — but that re-sweep grepped each Band A pattern's own named wordings, not the underlying claim, so it still missed three back-pressure comment sites (2.2's preamble) spelling the identical unconditional landing-queue claim as "the human's queue" / "awaiting a human" rather than any wording already grepped for; caught and reworded in PR #748's review round instead, with the level-aware qualification requirement 2.2's own spec passage already carries | 15 (3 added in that round) |
...
| "a human is waiting to land it" / "a human can merge" / "the human's merge click", asserted unconditionally for a landing decision that is actually `merge_autonomy`-gated | `lib/handoff.sh`, `lib/merge-queue.sh`, `lib/work-gone.sh`, `lib/claim.sh`, `lib/github-limit.sh`, `agent-cycle.sh`'s back-pressure claim-counting comment (the caller's own copy of `lib/claim.sh`'s), its listing-truncation comment and both halves of its 2.2a/2.2b back-pressure commentary, plus — spelled "the human's queue" / "awaiting a human" rather than any of the above wordings, and missed until PR #748's review round — three sites in requirement 2.2's own preamble (the deferred-stand-down paragraph, the reviewDecision-exclusion paragraph, the four-part-split paragraph) and `lib/handoff.sh`'s own file header (lines 3-9, whose body sites the same pass did reword), `docs/IMPLEMENTATION-PIPELINE-SPEC.md` (requirement 15d, the cost-profile passage, the abandoned-drafts/merge-conflicts/dequeued design decisions, requirements 2.2/2.2a/2.2b, D17's queue-lifecycle note, the Gotchas table), `README.md`'s pipeline overview and architecture diagram, `dashboard/index.html` | Names `merge_autonomy`'s ladder ("a human's click at `merge_autonomy: human`, or the Script's own arming step at `agent-merges-routine` and above") or drops the actor for a landing-state fact ("otherwise ready to land", "a landable PR") |
| "awaiting our reply to a human's review" / "no human is waiting on us", asserted unconditionally for a `review_feedback` candidate that is actually populated by any blocking review — a human's, or, at `agent-approves` and above, the Approver App's own `REQUEST_CHANGES` (missed until PR #748's review round) | `prompts/coordinator.md:108-114` | Names both sources ("a human's, or, at `agent-approves` and above, the Approver App's own `REQUEST_CHANGES`") and drops the actor from the empty-array reading ("nothing is waiting on our reply") |
...
| "the queue the human actually reads" / "the human's queue", said of the **re-request** a `review-feedback` round ends with | Reads like the Band A landing claims but is not: `_handoff_blocking_reviewers` reads `_handoff_latest_reviews`, which filters bot accounts out (requirement 34a — "not a reviewer to re-request … the one reviewer that is not the human this exists to reach"), so the re-request only ever names a non-bot account at any level. The queue it returns the pull request to really is a human's | `lib/handoff.sh:468`, `docs/IMPLEMENTATION-PIPELINE-SPEC.md:7575`, `prompts/implementer.md:107`, `prompts/reviewer.md:588` |
| The dashboard back-pressure card's "waiting on human" figure and the prose describing it | Accurate as-built: the card's own `humanQueueN` applies no `merge_autonomy` test, so its prose describes what it computes. The card diverging from requirement 2.2's level-aware gate at `agent-merges-routine` and above is a behaviour gap, not a wording one — filed as `tech-debt/TD-PPagop-26082508.md` rather than papered over by reworded prose | `dashboard/index.html:2872,2902`, `docs/DASHBOARD-SPEC.md:846,850,1747`, `test/dashboard-render.test.sh`, `test/claim.test.sh` |
| A pull request only a person can land, at every level | These read like the Band A landing claims above but are not: the Script's own arming step refuses both states outright, so no `merge_autonomy` level puts a second actor behind them. An unreconciled human comment since the ready flip stops arming (requirement 8d, `test/landing-wiring.test.sh`); a dequeued pull request is never re-armed (gate 5), so a re-queue really is a person's act | `agent-cycle.sh`'s two `revert: "failed"` warnings and its gate-5 dequeue comment, `prompts/coordinator.md`'s and `prompts/implementer.md`'s "only a human can re-queue" |
