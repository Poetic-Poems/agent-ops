...
filed: 2026-07-28
resolved: 2026-08-01
ref: https://github.com/Poetic-Poems/agent-ops/pull/149
---
...
Resolution note (2026-08-01): the fix sketch above says "bare item ids, like
`unblocked`", and the review on #149 found that shape fails in the unsafe
direction — a bare id folds into every same-numbered blocked item across
repos, and unlike an over-broad `unblocked` (which merely re-admits a
candidate) this marker *suppresses* a mandated re-read. What shipped is
`{item, repo}`, both off the `blocked` entry being re-checked; a bare id is
tolerated as a degraded fallback (requirement 33).
