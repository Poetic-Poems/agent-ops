...
any changes, make your own dedicated shallow clone of `origin/main` and work
...
```bash
git clone --depth 1 https://github.com/Poetic-Poems/agent-ops.git <scratch-dir>/agent-ops
```

Deepen the clone if the task turns out to need history — for example, a
rebase onto a `main` that has moved needs the merge base
(`git fetch --unshallow`, or `git fetch --depth=<n>`). Commit, push the
feature branch, and open the pull request from that clone; delete the clone
