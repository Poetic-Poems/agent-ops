...
assert_eq "the target role wants every label the pipeline applies" \
  "autonomous-agent enabler-escalation needs-refinement refined unvoided blocked blocked:needs-refinement obsolete complexity:low complexity:medium complexity:high" \
...
assert_eq "a label switched off by an empty value is not created" \
  "autonomous-agent enabler-escalation blocked blocked:needs-refinement obsolete complexity:low complexity:medium complexity:high" \
  "$(labels_catalogue "$tmp/config.json" "$SCHEMA" target | cut -f1 | tr '\n' ' ' | sed 's/ $//')"
...
assert_eq "a partly-labelled repository gets only what it is missing" \
  "enabler-escalation needs-refinement refined unvoided blocked:needs-refinement" \
  "$(cut -f2 <<<"$out" | tr '\n' ' ' | sed 's/ $//')"
...
assert_eq "and the labels either side of it are still created" \
  "autonomous-agent enabler-escalation needs-refinement refined blocked blocked:needs-refinement obsolete complexity:low complexity:medium complexity:high" \
