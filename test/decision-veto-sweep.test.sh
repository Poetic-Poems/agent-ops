...
# friends): that a successfully-recorded veto re-block also logs an
# `escalated` event naming the same log issue, `decision: true` — the
# registration that makes `ENABLER_ELIGIBLE_JQ` (lib/cycle-state.sh, see
# test/enabler-eligibility.test.sh's own decision-veto-tie cases) treat an
# open veto as a mechanical hold and its close as an immediate release. No
# escalated event should ever be logged for an item whose block was not
# actually recorded (already blocked, a malformed entry, …) or that does not
# match the veto this cycle's own `vetoed` action just reported.
#
...
# malformed): the two cases this file's own escalated-registration gate must
# tell apart, since the real recorder's own refusal logic is covered
# elsewhere (test/dependency-block-refusal.test.sh and neighbours).
RECORD_RESULT=0
RECORD_CALLS_FILE="$tmp_dir/record_calls"
reset_record_calls() { rm -f "$RECORD_CALLS_FILE"; RECORD_RESULT=0; }
...
# --- Case 2: the recorder refuses (already blocked) — no escalated event is
# logged for a block that was never actually recorded ------------------------
reset_log_event_calls
reset_record_calls
RECORD_RESULT=1
run "$actions"
...
assert_eq_n "logs no escalated event when the block was refused" "0" "^escalated" "$calls"
