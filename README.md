...
| `max_chained_cycles` | `3` | The most cycles that may run back-to-back in one lineage — the cron-fired original plus its immediate continuations, instead of each waiting for the next cron firing. In practice a productive cycle chains to this cap regardless of remaining work (the "sources remain" gate counts enabled source categories, which back-pressure never empties), so each productive cycle may buy up to `max_chained_cycles − 1` further full Co-Ordinator passes — the accepted price of the drain rate. `1` disables chaining. |
