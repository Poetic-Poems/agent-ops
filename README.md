...
| `project_review.defaults.report_directory` | *(unset)* | Optional. Where the review pipeline writes its report set (`README.md`, `01-summary.md`, ...) and reads past ones from — a GNU `date` format string, resolved with `date -u +"<format>"` relative to the repo root, e.g. `docs/reviews/project-review-%Y-%m-%d`. Absent everywhere, `reviews/project-review-%Y-%m-%d` is used, unchanged. |
