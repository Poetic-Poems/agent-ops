...
| `pager_repo` | `Pullwright/agent-ops` | Where the pager framework's own `pw::pager` issues are filed. Empty falls back to `crash_loop_repo`, which itself falls back to filing nothing — the value shown is this installation's own repository, not a generic default. |
