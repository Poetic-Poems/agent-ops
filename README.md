...
| `merge_autonomy_routine_sources` | `["register-hygiene", "tech-debt"]` | D18 WI-7: which work sources may be armed automatically at `agent-merges-routine` and above — a pull request also needs `complexity:low`/`medium` and to touch no protected path. A `repos[]` entry may override this per repository — see [Extended notes: `repos`](#extended-notes-repos). Matched against a pull request's own `source` by exact string equality: every `issues:<band>` work order's own `source` is the plain word `issues`, so an `issues:<band>` entry here never matches...[continued below](#extended-notes-merge_autonomy_routine_sources) |
