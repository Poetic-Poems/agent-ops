...
| `notify_min_interval_seconds` | `600` | How long, per notify key, between two pushes — so a fact that keeps repeating (the same dead-credential escalation refiling every cycle) arrives as one message and a count, not one per cycle. `0` disables coalescing outright: every eligible event posts. |
